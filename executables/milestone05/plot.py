...
def plot_profile_overlays(rho_files: list[Path], ux_files: list[Path], plots_dir: Path) -> None:
    ...
    for ux_path in ux_files:
        ...
        ux = load_csv(ux_path)
        velocity_profiles.append((step, ux.mean(axis=1)))

    for rho_path in rho_files:
        step = step_from_name(rho_path)
        rho = load_csv(rho_path)
        density_profiles.append((step, rho.mean(axis=1)))
    ...
    if rho_min == rho_max:
        pad = max(1e-6, 0.01 * abs(rho_min))
    else:
        pad = 0.1 * (rho_max - rho_min)
...
def main() -> None:
    ...
    parser.add_argument("--input-dir", default="outputs_m5", help="Directory containing Milestone 05 output CSV files")
    ...
    ux_steps = [step_from_name(path) for path in ux_files]
    uy_by_step = {step_from_name(path): path for path in uy_files}

    missing = [step for step in ux_steps if step not in uy_by_step]
    if missing:
        raise SystemExit(f"Missing matching uy files for steps: {missing}")

    last_ux_path = ux_files[-1]
    last_step = step_from_name(last_ux_path)
    last_uy_path = uy_by_step[last_step]

    ux_last = load_csv(last_ux_path)
    uy_last = load_csv(last_uy_path)
    ...
    print(f"Generated Milestone 05 plots in {plots_dir}")
